...
public:
  ...
  //获取错误码
  int GetLastErrCode() { return rdPrPack_.GetLastErrCode(); }
...
private:
  ...
  void PushDataWithLoopMode(bool &isTurn, std::list<node_info> &loopNodeList,
                            node_info &node_cur);
  ...
  void GrabScanDataWithNoLoop(std::list<node_info> &nodeList,
                              node_info *nodebuffer, size_t buffLen);

  void GrabScanDataWithLoop(std::list<node_info> &nodeList,
                            node_info *nodebuffer, size_t buffLen);
  void PushValidData2Buffer(node_info &nodeInfo, int index,
