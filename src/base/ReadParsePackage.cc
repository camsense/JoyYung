...
#define FPS_MAX             2160  //最大fps
#define FPS_MIN             2000  //最小fps
#define FPS_CONTINUE_SECOND 5     // fps异常的持续时间

#define SPEED_MAX             400  //最大速度
#define SPEED_MIN             240  //最小速度
#define SPEED_CONTINUE_SECOND 10   //最大速度或者最小速度异常的持续时间

ReadParsePackage::ReadParsePackage() { m_lastErrorCode = 0; }
...
///////////2023-06-02
void ReadParsePackage::checkBlockSpeed(unsigned int u16speed)
{
	if (u16speed <= MIN_BLOCK_SPEED) {
		if (m_u64checkBlockSpeed != 0) 
		{
			int64_t endTime = GetTimeStamp();
			if ((endTime - m_u64checkBlockSpeed) / 1e6 >= BLOCK_SPEED_CONTINUE_MS) {
				m_lastErrorCode = ERR_SHARK_MOTOR_BLOCKED;
				m_u64checkBlockSpeed = 0;
			}

		}
		else {
			m_u64checkBlockSpeed = GetTimeStamp();
		}
	}
	else {
		m_u64checkBlockSpeed = 0;
...
int ReadParsePackage::ParseLidarData(BYTE* in_buffer, const int in_numData, rangedata* out_dataPack) {
  ...
  // std::cout << "time:" << GetTimeStamp() / 1.0e6 << "\tbuffer_speed:" << speed << std::endl;
  ...
  for (int i = 0; i < in_numData; ++i) {
    ...
    // std::cout << "dist: " << out_dataPack[i].dist << "\tangle: " << out_dataPack[i].angle
    //           << "\tstrenth: " << (int)(out_dataPack[i].syn_quality) << std::endl;
  }
