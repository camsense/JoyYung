...
#define CIRCLE_NUMBER_MAX 401 //注意:2000转设置800  3000转设置1200
...
int Dev::Initialize(char *port, unsigned int baud,
                    bool isGetLoopData /*false*/) {
...
void Dev::PushDataWithLoopMode(bool &isTurn, std::list<node_info> &loopNodeList,
                               node_info &node_cur) {
  if (isTurn) {
    if (!isGreaterThan ||
        (isGreaterThan && loopNodeList.size() > LESS_THAN_NUMBER)) {
      std::lock_guard<std::mutex> guard(mutex_);
      m_nodeList.clear();
  ...
  while (is_scanning_) {
    ...
    for (int i = 0; i < rdPrPack_.data_num_per_pack_; ++i) {
      ...
      if (0 != dist && !dataPack[i].flag) { //&& !dataPack[i].flag
        ...
        double angle_correct_cur =
            angle_cur + 80.0 - atan(dist / CENTER_BASE_LINE) / PI * 180;
        ...
        dataPack[i].dist =
            sqrt(CENTER_BASE_LINE * CENTER_BASE_LINE + 1.0 * dist * dist);
      }
...
void Dev::GrabScanDataWithLoop(std::list<node_info> &nodeList,
                               node_info *nodebuffer, size_t buffLen) {
...
void Dev::PushValidData2Buffer(node_info &nodeInfo, int index,
                               node_info *nodebuffer, int len) {
  bool isExit = false;
  while (!isExit) {
    index = index % len;
...
void Dev::GrabScanDataWithNoLoop(std::list<node_info> &nodeList,
                                 node_info *nodebuffer, size_t buffLen) {
  size_t index = 0;
  size_t indexCount = 0;
  size_t startIndex = 0;
