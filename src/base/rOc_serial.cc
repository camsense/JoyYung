...
char rOc_serial::openDevice(const char *Device, const unsigned int Bauds) {
  ...
  case 128000:
    ...
    break;
  case 153600:
    dcbSerialParams.BaudRate = 153600;
    break;
  case 256000:
...
int rOc_serial::readData(unsigned char *Buffer, unsigned int expectedBytes) {
#if defined(_WIN32) || defined(_WIN64)
  if (hSerial == INVALID_HANDLE_VALUE) {
    return -5;
  }

  timeouts.ReadTotalTimeoutConstant = 10;    // Set the TimeOut
  if (!SetCommTimeouts(hSerial, &timeouts))  // Write the parameters
  ...
  if (nfds == 0) {
    ...
    nread = read(fd, Buffer, expectedBytes);  //即使不满desire_get_len，也会返回实际读取到的数据量
    return nread;
  }
