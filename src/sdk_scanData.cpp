...
int main(int argc, char **argv) {
  ...
  while (!isExit) {
    /*      node_info nodebuffer[2048];
       ...
       / 64.0f, nodebuffer[i].distance_q2 / 4, nodebuffer[i].isValid,
          ...
          out.write("\n", 1);
          char headTail[64] = {0};
          sprintf(headTail, "%.4f,%.4f,%d\n", nodebuffer[0].angle_q6_checkbit
       / 64.0f, nodebuffer[count - 1].angle_q6_checkbit / 64.0f, count);
    ...
    //   it->distance_q2 / 4, it->isValid,
    //           it->syn_quality);
    //   // out.write(buff, strlen(buff));
    // }

    long long e = GetTimeStamp();
    long long timeStamp = (e - b) / 1000000;
    b = e;
    char buff[128] = {0};

    // sprintf(buff, "%d, %lld,\n", dataList.size(), timeStamp);
    // out1 << buff;
